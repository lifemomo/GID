...
import numpy as np  # 正确导入numpy
import sys
import os

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../utils")))  # 确保正确导入 evaluate
import evaluate  # 直接导入，不用相对路径
import torch.distributed as dist  # 计划未来支持多 GPU 训练，确保 dist 被正确导入
...
class SingleRunner:
    def parse_runner_args(parser):
        ...
        parser.add_argument("--test_epoch", type=int, default=1,
                            help='test once for how many epochs, 0 for no test during training.')
        ...
        parser.add_argument("--test_filtered", type=int, default=0,
                            help='whether filter out the items in the training data.')
        parser.add_argument("--test_filtered_batch", type=int, default=1,
                            help='whether testing with filtered data in batch.')

        return parser

    def __init__(self, model, tokenizer, train_loader, valid_loader, device, args):
        self.model = model
        self.rank = args.rank if hasattr(args, 'rank') else 0  # 如果 args 有 rank 参数就用它，没有就默认是 0
        ...
        self.best_score = -float('inf')  # 初始化最优分数，越大越好（比如 ndcg@10）
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.test_epoch > 0:
                if (epoch + 1) % self.test_epoch == 0:
                    ...
                    self.model.eval()
                    self.test()
            if dist.is_available() and dist.is_initialized():  # 仅在多 GPU 训练时执行
                dist.barrier()
            # dist.barrier()
        if self.valid_select > 0:
            if self.rank == 0:
    ...
    def get_testloader(self):
        ...
        for dataset in datasets:
            for task in tasks:
                ...
                testloader = DataLoader(dataset=testdata, batch_size=self.args.eval_batch_size, collate_fn=collator,
                                        shuffle=False)
                self.testloaders.append(testloader)

    # def test(self, path=None):
    #     # ✅ 解包模型以调用 generate()，必须写在最前面
    ...
    def test(self, path=None):
        ...
        for loader in self.testloaders:
            if self.test_filtered > 0:
                if self.test_filtered_batch > 0:
                    self.test_dataset_task_filtered_batch(loader, real_model)
                else:
                    assert self.args.eval_batch_size == 1
                    self.test_dataset_task_filtered(loader, real_model)
            else:
                self.test_dataset_task(loader, real_model)
    ...
    def test_dataset_task_filtered_batch(self, testloader, model):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(testloader):
                ...
                # model = self.model.module if isinstance(self.model,
                #                                         torch.nn.parallel.DistributedDataParallel) else self.model
                ...
                rel_results = evaluate.rel_results_filtered(testloader.dataset.positive, testloader.dataset.id2user,
                                                            user_idx, \
                                                            self.generate_num + testloader.dataset.max_positive, \
                                                            generated_sents, gold_sents, prediction_scores,
                                                            self.generate_num)
    ...
    def test_dataset_task(self, testloader, model):
        ...
        with torch.no_grad():
            ...
            if not hasattr(self, 'best_score'):
                ...
                self.best_model_path = None  # 还没有最佳模型
            ...
            if current_score > self.best_score:
                ...
                model_save_path = os.path.join(
                    ...
                    f"best_epoch{self.current_epoch:02d}_ndcg{current_score:.4f}.pt"
                )

                # 删除旧的 best 模型（如果存在）
                if hasattr(self, 'best_model_path') and self.best_model_path is not None:
                    if os.path.exists(self.best_model_path):
                        try:
                            os.remove(self.best_model_path)
                            logging.info(f"🗑️ Deleted previous best model: {self.best_model_path}")
                        except Exception as e:
                            logging.warning(f"Failed to delete previous best model {self.best_model_path}: {str(e)}")

                # 保存新的 best 模型
                torch.save(model_to_save.state_dict(), model_save_path)
                logging.info(f"🌟 Best model saved at {model_save_path} with ndcg@10 = {current_score:.4f}")
