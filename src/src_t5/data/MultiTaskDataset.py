...
class MultiTaskDataset(Dataset):
    def parse_dataset_args(parser):
        ...
        parser.add_argument("--item_indexing", type=str, default='sequential',
                            help="item indexing method, including random, sequential and collaborative")
        parser.add_argument("--tasks", type=str, default='sequential,direct,straightforward',
        ...
        parser.add_argument("--prompt_file", type=str, default='../prompt_template.txt',
        ...
        parser.add_argument("--collaborative_token_size", type=int, default=200,
                            help='the number of tokens used for indexing')
        parser.add_argument("--collaborative_cluster", type=int, default=20,
                            help='the number of clusters in each level for collaborative indexing.')
        parser.add_argument("--collaborative_last_token", type=str, default='sequential',
                            help='how to assign the last token to items within the same clusters, random or sequential')
        parser.add_argument("--collaborative_float32", type=int, default=0,
                            help='1 for use float32 during indexing, 0 for float64.')
        ...
        parser.add_argument("--valid_prompt", type=str, default='seen:0',
                            help='The prompt used for evaluation, seen/unseen: id')
        parser.add_argument("--valid_prompt_sample", type=int, default=1,
                            help='use sampled prompt for validation every epoch.')
        parser.add_argument("--valid_sample_num", type=str, default='3,3',
                            help='the number of sampled data for each task')
        parser.add_argument("--test_prompt", type=str, default='seen:0',
                            help='The prompt used for evaluation, seen/unseen: id')
    ...
    def __init__(self, args, dataset, mode):
        ...
        # 添加 collaborative_sparse 属性并设置默认值
        self.collaborative_sparse = 0  # 假设使用默认值
        ...
        # print("📌 实际读取的 user_sequence.txt 路径为：", os.path.join(self.data_path, self.dataset, 'user_sequence.txt'))
        ...
        if args.distributed:
            if self.item_indexing == 'sequential':
                if self.rank == 0:
                    ...
                    indexing.sequential_indexing(self.data_path, self.dataset, self.user_sequence_dict,
                                                 args.sequential_order)
                    dist.barrier()
                else:
                    dist.barrier()
                self.reindex_user_seq_dict, self.item_map = indexing.sequential_indexing(self.data_path, self.dataset,
                                                                                         self.user_sequence_dict,
                                                                                         args.sequential_order)
                ...
                if self.rank == 0:
                    ...
                    dist.barrier()
                self.reindex_user_seq_dict, self.item_map = indexing.random_indexing(self.data_path, self.dataset,
                                                                                     self.user_sequence_dict)
            elif self.item_indexing == 'collaborative':
                if self.rank == 0:
                    logging.info(
                        f"Reindex data with collaborative indexing method with token_size {self.collaborative_token_size} and {self.collaborative_cluster_num} cluster")
                    indexing.collaborative_indexing(self.data_path, self.dataset, self.user_sequence_dict,
                                                    self.collaborative_token_size, \
                                                    self.collaborative_cluster_num, self.collaborative_last_token,
                                                    self.collaborative_float32)
                    dist.barrier()
                else:
                    dist.barrier()
                self.reindex_user_seq_dict, self.item_map = indexing.collaborative_indexing(self.data_path,
                                                                                            self.dataset,
                                                                                            self.user_sequence_dict, \
                                                                                            self.collaborative_token_size,
                                                                                            self.collaborative_cluster_num, \
                                                                                            self.collaborative_last_token,
                                                                                            self.collaborative_float32)
                self.new_token = []
            ...
            if self.item_indexing == 'sequential':
                ...
                logging.info(
                    f"Reindex data with collaborative indexing method with token_size {self.collaborative_token_size} and {self.collaborative_cluster_num} cluster")
                self.reindex_user_seq_dict, self.item_map = indexing.collaborative_indexing(
                    self.data_path, self.dataset, self.user_sequence_dict, self.collaborative_token_size,
