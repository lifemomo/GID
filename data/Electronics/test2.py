...
# ---------- 文件路径 ----------
item_old_file = "item_collaborative_indexing_500_20_sequential-old.txt"
item_new_file = "item_collaborative_indexing_500_2_sequential.txt"
user_seq_old_file = "user_sequence_collaborative_indexing_500_20_sequential-old.txt"
user_seq_new_file = "user_sequence_collaborative_indexing_500_2_sequential.txt"
...
"检查作者交互顺序和user_sequence.txt中的交互顺序是否一致"
import json

# raw_file   = "user_sequence.txt"
# collab_file= "user_sequence_collaborative_indexing_500_20_sequential-old.txt"
# user_map_f = "user_indexing.txt"
# item_map_f = "item_collaborative_indexing_500_20_sequential-old.txt"
#
# # 读映射
# u_map = {}
# for line in open(user_map_f, 'r', encoding='utf-8'):
#     p = line.strip().split()
#     if len(p)==2: u_map[p[0]] = p[1]
#
# i_map = {}
# for line in open(item_map_f, 'r', encoding='utf-8'):
#     p = line.strip().split()
#     if len(p)==2: i_map[p[0]] = p[1]
#
# # 原始 -> 映射 后得到 dict[user]->list(items)
# def load_raw_as_mapped(path):
#     d = {}
#     miss_users, miss_items = 0, 0
...
#         mi = []
#         for it in items:
#             if it in i_map: mi.append(i_map[it])
#             else: miss_items += 1
#         d[mu] = mi
#     return d, miss_users, miss_items
#
# # collaborative 文件 -> dict[user]->list(items)
# def load_collab(path):
#     d = {}
#     for line in open(path, 'r', encoding='utf-8'):
#         toks = line.strip().split()
#         if not toks: continue
...
#     return d
#
# raw_mapped, miss_u, miss_i = load_raw_as_mapped(raw_file)
# collab = load_collab(collab_file)
#
# print(f"覆盖统计: 缺失用户映射 {miss_u} 行，缺失物品映射 {miss_i} 个")
# common_users = set(raw_mapped) & set(collab)
# print(f"公共用户数: {len(common_users)} / raw_mapped={len(raw_mapped)} / collab={len(collab)}")
#
# mismatch = []
# for u in common_users:
#     if raw_mapped[u] != collab[u]:
#         mismatch.append(u)
#
# print(f"不一致用户数: {len(mismatch)}")
# if mismatch[:5]:
#     for u in mismatch[:5]:
#         print("[Mismatch]", u)
#         print("exp:", raw_mapped[u][:15], "... len=", len(raw_mapped[u]))
#         print("got:", collab[u][:15],     "... len=", len(collab[u]))
