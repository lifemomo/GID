...
#
# # -------------------- 读取映射和数据 -------------------- #
# with open('index_to_business_id.json', 'r', encoding='utf-8') as f:
#     index_to_business_id = json.load(f)
#
...
#     [[id_mapping[i] for i in edge_pair] for edge_pair in graph_data['edge_index'].T],
#     dtype=torch.long
# ).T
...
#     pos_sim = torch.sum(normed[row] * normed[col], dim=-1) / tau
#     pos_sim_exp = torch.exp(pos_sim)
#     sim_matrix = torch.mm(normed, normed.t()) / tau
...
#
#
# def train_gcn_contrastive_loss(data, hidden_dim=32, out_dim=16, epochs=100, lr=0.01, patience=10):
#     model = GCN(data.x.size(1), hidden_dim, out_dim)
#     optimizer = torch.optim.Adam(model.parameters(), lr=lr)
...
#         model.train()
#         optimizer.zero_grad()
#         emb = model(data.x, data.edge_index, data.edge_weight)
#         loss = contrastive_loss(emb, data.edge_index)
#         loss.backward()
#         optimizer.step()
#
#         if loss.item() < best_loss:
#             best_loss = loss.item()
#             best_embeddings = emb.detach().numpy()
#             counter = 0
#         else:
#             counter += 1
#             if counter >= patience:
...
#
# # -------------------- 构建聚类树 -------------------- #
# def build_cluster_structure_hierarchical(embeddings, max_depth=2):
...
#
# cluster_structure = build_cluster_structure_hierarchical(final_embeddings, max_depth=2)
#
# # -------------------- 统计最大簇大小作为 n -------------------- #
# def get_max_leaf_cluster_size(cluster_structure):
#     max_size = 0
#     queue = deque([cluster_structure])
#     while queue:
#         node = queue.popleft()
#         if node["leaf"]:
#             max_size = max(max_size, len(node["indices"]))
#         else:
#             queue.extend(node["children"])
#     return max_size
#
# n = get_max_leaf_cluster_size(cluster_structure)
#
# # -------------------- 编制路径索引 -------------------- #
# def assign_paths_bfs(tree_root, n):
#     global_counter = [0]
#     def get_next_index():
...
#
#         for node_name, node_dict in sample_nodes:
...
#     return cluster_tree
#
# cluster_tree = assign_paths_bfs(cluster_structure, n)
#
# # -------------------- 输出与规范化 -------------------- #
# with open(f"cluster_tree_{n}_hierarchical.json", 'w', encoding='utf-8') as f:
...
#             original_item_id = index_to_business_id[str(idx)]
#             item_map[f"{original_item_id}"] = token
#
# original_output = f"item_collaborative_indexing_original_{n}_hierarchical.txt"
...
#             outfile.write(line)
#
# processed_output = f"item_collaborative_indexing_{n}_hierarchical_sequential.txt"
# process_item_index_file(original_output, processed_output)
...
output_file = f"item_collaborative_indexing_{n}_hierarchical_sequential.txt"
